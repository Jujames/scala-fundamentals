...
import main.scala.com.Project.entities._
import main.scala.com.Project.services.Currency._
...
trait AccountService extends AccountsDb with CustomerService with ProductService  {
  ...
  def openLendingAccount(customerId: UUID, productId: UUID, balance: Dollars = Dollars(0)): UUID = {
   require(getCustomer(customerId).nonEmpty, s"no customer found with id=$customerId")
    ...
    saveLendingAccounts(account)
    account.id
  }

  def deposit(accountId: UUID, dollars: Dollars): Unit = {
    val maybeAccount = getDepositAccount(accountId)
    require(maybeAccount.nonEmpty, "A valid deposit account Id must be provided")
  ...
  def useCreditCard(accountId: UUID, dollars: Dollars): Unit = {
    ...
    require(maybeAccount.nonEmpty, "A valid lending account Id must be provide")
    maybeAccount.get withdraw dollars
  }
